...
```clawr
func score(rolls: [integer @range(0..<10)]) {
    ...
    func frameScore(at index: integer @range(0..<10)) => when {
        isStrike(at: index) => rolls[index] + rolls[index + 1] + rolls[index + 2]
        isSpare(at: index) => rolls[index] + rolls[index + 1]
	    _ -> rolls[index]
    }
    func isStrike(at frameIndex: integer @range(0..<10)) => rolls[frameIndex] == 10
    func isSpare(at frameIndex: integer @range(0..<10)) => rolls[frameIndex] + rolls[frameIndex + 1] == 10
}
```
